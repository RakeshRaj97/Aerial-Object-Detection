...
class TestEngine:
    ...
    def __call__(self):
        ...
        for c in tqdm(self.params.class_label_map):
            ...
            matched_det = bb.stat.match_det(det_c, anno_c, self.iou, criteria=bb.stat.coordinates.iou, ignore=bb.stat.IgnoreMethod.SINGLE)
    ...
    def test_loss(self):
        ...
        with torch.no_grad():
            for idx, (data, target) in enumerate(tqdm(self.dataloader)):
                ...
                loss_dict['tot'].append(self.loss.loss.item() * num_img)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-v', '--validation', help='Use validation instead of test-set', action='store_true')
    ...
    if args.thresh is not None: # Overwrite threshold
        params.post['getdarknetboxes'].conf_thresh = args.thresh

    # Dataloader
    data = params.val_set if args.validation else params.test_set
